...
- Database access only through the service layer (`services/`); never call the Supabase client directly from a component or page; never expose `SUPABASE_SERVICE_ROLE_KEY` to the browser (`Coding-Standards.md`, `security.md` §12). **Exception, added during pre-flight review:** this rule governs the application's request path (UI → `services/` → Supabase). It does not apply to `scripts/bootstrap-owner.ts` (Task 4, an offline admin tool the spec §6.5 explicitly describes as using the admin client directly) or to test setup/teardown code in `tests/integration/` and `tests/e2e/` (Tasks 15–16, which create and clean up throwaway fixture data directly via the admin client, per standard test-fixture practice). Task reviewers for 4, 15, and 16 should not flag direct admin-client `.from(...)` calls in those files as a Global Constraints violation.
