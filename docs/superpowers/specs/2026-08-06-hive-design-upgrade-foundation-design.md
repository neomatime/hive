...
- **Dialog (new shared component):** currently hand-rolled per call site (`task-detail-dialog.tsx` and the team-member profile editor each duplicate `fixed inset-0 z-50 grid place-items-center bg-black/40` + `role="dialog"` markup). Consolidate into `components/ui/dialog.tsx` — this directly serves the dark-mode pass, since otherwise the same styling fix would need to land in 3+ places independently. Modals keep a real shadow (`--shadow-lg`) — the one deliberate, already-documented exception to "hairlines not shadows," since an overlay needs actual elevation to read as above the page.
- **Tabs — deferred, but real consumers exist (correction from an earlier draft of this section).** `components/projects/project-shell.tsx` has a genuine tab bar (project sub-navigation: overview/board/files/calendar/activity/settings) with no active-state indicator at all today. `components/tasks/my-tasks-dashboard.tsx` and `components/notifications/notification-inbox.tsx` both have filter-pill rows using the exact "filled pills" pattern this bullet originally called out to replace. Deferring the *shared component* out of this Badge/Dialog primitives PR is still the right scope call -- fixing `project-shell.tsx`'s missing active state and normalizing the filter-pill rows is real, separate, focused work, not something to fold into a component-extraction PR. But it's real, identified work, not a nonexistent need -- track it as its own follow-up task rather than letting it drop.
